...
public:
     ...
     */
    bool Listen(Connection& conn) REQUIRES(!m_mutex);
     ...
     * @return true on success
     */
    bool Connect(const CService& to, Connection& conn, bool& proxy_error) REQUIRES(!m_mutex);
...
private:
     ...
     */
    void CheckControlSock() REQUIRES(!m_mutex);
