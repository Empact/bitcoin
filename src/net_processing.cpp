...
bool PeerLogicValidation::MarkBlockAsInFlight(NodeId nodeid, const uint256& hash, const CBlockIndex* pindex, std::list<QueuedBlock>::iterator** pit)
{
    CNodeState *state = State(nodeid);
    assert(state != nullptr);
...
bool PeerLogicValidation::AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    switch (inv.type)
    {
    case MSG_TX:
...
bool PeerLogicValidation::ProcessHeadersMessage(CNode* pfrom, const std::vector<CBlockHeader>& headers, bool via_compact_block)
{
    const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
    ...
    BlockValidationState state;
    if (!ProcessNewBlockHeaders(headers, state, m_chainparams, &pindexLast)) {
        if (state.IsInvalid()) {
...
void PeerLogicValidation::ProcessOrphanTx(std::set<uint256>& orphan_work_set, std::list<CTransactionRef>& removed_txn) EXCLUSIVE_LOCKS_REQUIRED(cs_main, g_cs_orphans)
{
    AssertLockHeld(cs_main);
...
bool PeerLogicValidation::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::ADDR) {
        ...
        for (CAddress& addr : vAddr)
        {
            ...
            if (m_banman->IsBanned(addr)) continue; // Do not process banned addresses beyond remembering we received them
    ...
    if (strCommand == NetMsgType::CMPCTBLOCK)
    {
        ...
        const CBlockIndex *pindex = nullptr;
        BlockValidationState state;
        if (!ProcessNewBlockHeaders({cmpctblock.header}, state, m_chainparams, &pindex)) {
            if (state.IsInvalid()) {
        ...
        if (fBlockReconstructed) {
            ...
            ProcessNewBlock(m_chainparams, pblock, /*fForceProcessing=*/true, &fNewBlock);
            if (fNewBlock) {
    ...
    if (strCommand == NetMsgType::BLOCK)
    {
        ...
        bool fNewBlock = false;
        ProcessNewBlock(m_chainparams, pblock, forceProcessing, &fNewBlock);
        if (fNewBlock) {
            pfrom->nLastBlockTime = GetTime();
        } else {
            LOCK(cs_main);
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    try
    {
        fRet = ProcessMessage(pfrom, strCommand, vRecv, msg.m_time, interruptMsgProc);
        if (interruptMsgProc)
            return false;
        if (!pfrom->vRecvGetData.empty())
            fMoreWork = true;
