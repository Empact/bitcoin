...
static size_t GetWitnessSigOpCount(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags)
{
...
int64_t GetTransactionSigOpCost(const CTransaction& tx, const CCoinsViewCache& inputs, int flags)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        assert(!coin.IsSpent());
        const CTxOut &prevout = coin.out;
        nSigOps += GetWitnessSigOpCount(tx.vin[i].scriptSig, prevout.scriptPubKey, &tx.vin[i].scriptWitness, flags);
    }
    return nSigOps;
}
